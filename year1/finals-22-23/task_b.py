...
class Task(TaskBase):
    ...
    def import_matrix(path: str = "Matrix.txt") -> np.ndarray:
        """The file Matrix.txt contains 49 numerical values. Read in the
        content and organise the values into a mathematical matrix A, with
    ...
    def create_matrix_c(matrix_a: np.ndarray) -> np.ndarray:
        ...
        Each element C0j of the new first row will contain the largest value
        of the respective column in A.
        Each element Ci0  of the new first column will contain the smallest
        value of the respective row in A
        """
    ...
    def diagonals(matrix: np.ndarray) -> tuple[float, float]:
        """Write a function, Diagonals(), to calculate the sum of all the
        elements in the diagonal of C and the sum of all the elements in the
        anti-diagonal of C. The function receives a matrix as input argument
        and returns a tuple with two values as output argument, namely
    ...
    def sum_det_minor(matrix: np.ndarray) -> float:
        ...
        # iterate over all the elements of the matrix and compute the
        # determinant of the minor matrix, then sum all the determinants
        return sum(
            (
    ...
    def task1(self) -> dict[str, np.ndarray]:
        ...
        4. Compute the sum of the determinants of all the minor matrices that
        can be obtained from C
        5. Subdivide C in four sub-quadrants. Compose a new matrix D, of same
        size as C, where each sub-quadrant is obtained by transposing the
        homologous sub-quadrant of C"""
        mat_a: np.ndarray = self.import_matrix("year1/finals-22-23/Matrix.txt")
        print(mat_a)
        mat_c: np.ndarray = self.create_matrix_c(mat_a)
        print(mat_c)
        print(self.diagonals(mat_c))
        print(self.sum_det_minor(mat_c))
        mat_d: np.ndarray = self.subdivide_and_transpose(mat_c)
        print(mat_d)
        return {"matrix_d": mat_d}
...
def main() -> None:
    ...
    sub-quadrant of C"""
    mat_a: np.ndarray = import_matrix("year1/finals-22-23/Matrix.txt")
    print(mat_a)
