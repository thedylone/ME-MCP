...
class Task(TaskBase):
    ...
    def task1(self):
        ...
        self.matrix_a = np.loadtxt(
            "year1/finals-21-22/Matrix.txt", dtype=float
        ).reshape(600, 600)
    ...
    def task3(self) -> None:
        ...
        np.savetxt(
            "year1/finals-21-22/Averages.txt", self.matrix_avg.reshape(3600, 1)
        )
        np.savetxt(
            "year1/finals-21-22/Determinants.txt",
            self.matrix_det.reshape(3600, 1),
        )
        plt.imshow(self.matrix_a)
        plt.show()
