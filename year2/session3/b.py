...
class Task(TaskBase):
    ...
    def newt_div_diff(
        ...
        if len(xn) == 1:
            return yn[0]
        left: float = Task.newt_div_diff(xn[:-1], yn[:-1])
        right: float = Task.newt_div_diff(xn[1:], yn[1:])
    ...
    def newton_interp(
        ...
        for xp in x:
            ...
            for i in range(1, n):
                ...
                for j in range(i):
                    prod *= xp - xn[j]
                y += Task.newt_div_diff(xn[: i + 1], yn[: i + 1]) * prod
            out.append(y)
        return out
...
if __name__ == "__main__":
    ...
    quiz_p = Task.newton_interp(quiz_xn, quiz_yn, [0.8])
    print(quiz_p)
